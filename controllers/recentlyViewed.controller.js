  ...
  try {
    ...
    const response = await Promise.all(
      serviceDetails.flatMap(async (serviceDetail) => {
        const category = await Category.findById(serviceDetail.Category).select(
          "name"
        );
        return serviceDetail.services
          .filter((service) =>
            packageIds.some((pkgId) => pkgId.equals(service._id))
          )
          .map((service) => {
            const values =
              service.values instanceof Map
                ? Object.fromEntries(service.values)
                : service.values;
